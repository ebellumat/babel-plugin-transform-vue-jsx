...
<!-- The example row's hash is the real digest of its own four fields against
     https://github.com/vuejs/babel-plugin-transform-vue-jsx, so it does not match this repository's URL - it is an
...
npx proof-of-usage hash \
  ...
  --repo "https://github.com/vuejs/babel-plugin-transform-vue-jsx"
```

```bash
printf '%s' "ExampleModel v2:AI Corp:2026-08-12T14:30:00Z:https://github.com/vuejs/babel-plugin-transform-vue-jsx" | shasum -a 256
```

```python
import hashlib
hashlib.sha256("ExampleModel v2:AI Corp:2026-08-12T14:30:00Z:https://github.com/vuejs/babel-plugin-transform-vue-jsx".encode()).hexdigest()
```
...
2. **There.** In the credits your product already publishes — credits screen, about page,
   ...
   ```
   Includes material from vuejs/babel-plugin-transform-vue-jsx — https://github.com/vuejs/babel-plugin-transform-vue-jsx
